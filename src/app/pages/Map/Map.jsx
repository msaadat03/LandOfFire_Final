...
import data from "../../data/bakuData.json";
import { useEffect, useRef, useState } from "react";
import { useNavigate } from "react-router-dom";

export default function Map() {
  const mapSvgRef = useRef();
  const [selectedRegionKey, setSelectedRegionKey] = useState("1");

  const navigate = useNavigate();

  useEffect(() => {
    if (mapSvgRef.current) {
      for (let i = 0; i < mapSvgRef.current.children.length; i++) {
        let currentEl = mapSvgRef.current.children[i];

        currentEl.addEventListener("click", () => {
          if (currentEl.dataset.regionkey) {
            setSelectedRegionKey(currentEl.dataset.regionkey);
          }
        });
      }
    }
  ...
  useEffect(() => {
    if (mapSvgRef.current) {
      for (let i = 0; i < mapSvgRef.current.children.length; i++) {
        ...
        if (currentEl.dataset.regionkey == selectedRegionKey) {
          currentEl.style.fill = "#448243";
        } else {
          currentEl.style.fill = "#5E6D62";
        }
      }
    }
  }, [selectedRegionKey]);
      ...
      <Box component={Paper}>
        ...
        <Grid container sx={{ p: 7 }}>
          <Grid item xs={12} lg={6}>
            {data &&
              data.map((x) => {
                if (x.key != selectedRegionKey) return null;

                return (
                  <Box
                    key={x.key}
                    sx={{ maxWidth: 445, p: 3, background: "white", border: 1 }}
                  >
                    <Card
                      sx={{ maxWidth: 405, borderRadius: 0, boxShadow: "none" }}
                      key={x.key}
                    >
                      <CardMedia
                        sx={{ height: 220 }}
